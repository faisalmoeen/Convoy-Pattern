...
public class VcodaOptim {
	...
	static String outputFilePath1="C:/Users/buraq/Google Drive/Brussels/PhD Work/working-folder/experiments/trucks_dataset/convoysOutputExp.txt";
	static int m=3;
...
//		List<Convoy> Vpcc = PCCD(clusterMap,k,m);
		...
		//*******Print Vpcc************
		Utils.writeConvoys(Vpcc, outputFilePath1);

	}

	private static List<Convoy> experiment(HashMap<Integer, List<Cluster<PointWrapper>>> clusterMap, int k, int m) {
		int div=2;
		...
		List<Convoy> C = null;
		for(int t=minTime;t<=maxTime;t=t+(k/div)){//maxTime;t++){
			...
			for(Convoy c : C){
				c.setMatched(false);c.setAbsorbed(false);c.setStartTime(t);c.setEndTime(t);c.setRightOpen(true);
			...
			for(Convoy v : V){
				...
				for(Convoy c : C){
					if(c.size()>=m && v.intersection(c).size() >= m){ //extend (v,c)
						...
						Convoy vext = new Convoy(v.intersection(c),v.getStartTime(),t+(k/div)-1);
