...
public class MyKDTree {
	...
	private MyKDNode root = null; // the tree root
	...
	DistanceFunction distanceFunction = new MyEuclidianDistance(); 
	 ...
	 * @return a node
	 */
	private MyKDNode generateNode(int currentD, List<DoubleArray> points, int left, int right) {
		...
		if (right == left) {
			return new MyKDNode(points.get(left), currentD);
		}
		...
		MyKDNode node = new MyKDNode(medianNode, currentD);
		currentD++;
	 ...
	 */
	private void findParent(DoubleArray target, MyKDNode node, int d) {		
	...
	private void nearest(MyKDNode node, DoubleArray targetPoint) {
...
//	
	 ...
	 */
	private void findParent_knn(DoubleArray target, MyKDNode node, int d) {		
	 ...
	 */
	private void tryToSave(MyKDNode node, DoubleArray target) {
		if(node == null){
			return;
		}
	 ...
	 */
	private void nearest_knn(MyKDNode node, DoubleArray targetPoint) {
	 ...
	 * @param the radius
	 */
	private void findPointsWithinRadius(MyKDNode node, DoubleArray targetPoint, List<DoubleArray> result, double radius) {
...
//			if (targetPoint.data[dMinus1] < node.values.data[node.d]) {
	 ...
	 * @param the radius
	 */
	private void tryToSaveRadius(MyKDNode node, DoubleArray target, List<DoubleArray> result, double radius) {
		if(node == null){
			return;
		}
		double distance = distanceFunction.calculateDistance(target, node.values);
	 ...
	 * @param the radius
	 */
	private void findPointsWithinRadiusWithDistance(MyKDNode node, DoubleArray targetPoint, List<KNNPoint> result, double radius) {
		// if it is the target point, we skip it because we don't want to return it
		if(node.values != targetPoint) {
	 ...
	 * @param the radius
	 */
	private void tryToSaveRadiusWithDistance(MyKDNode node, DoubleArray target, List<KNNPoint> result, double radius) {
		if(node == null){
			return;
		}
		double distance = distanceFunction.calculateDistance(target, node.values);
	 ...
	 * @return a string
	 */
	private String toString(MyKDNode node, String indent){
		if(node == null){
			return "";
		}
